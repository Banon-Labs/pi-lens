...
prompt_file="$repo_root/docs/emoji-posterity-smoke-prompt.txt"
helper="/home/choza/projects/scripts/pi-kitty-smoke.sh"
title="pi-lens-emoji-posterity-sandbox"
out_dir=""
turn_timeout=300
quit_after=0
keep_dpi_shim=0
...
usage() {
	cat <<'EOF'
pi-lens-emoji-posterity-smoke.sh - launch a Kitty Pi smoke session that shows 🔴, 🟡, and ✅ in one prompt

Usage:
  pi-lens-emoji-posterity-smoke.sh [options]
...
if [[ -z "$out_dir" ]]; then
	out_dir=$(mktemp -d "/tmp/pi-lens-emoji-posterity-script-XXXXXX")
else
	mkdir -p "$out_dir"
fi
cleanup_artifact="$out_dir/summary.txt"
tool_result_artifact="$out_dir/emoji-posterity-tool-result.txt"
