...
`;
	...
	describe("scanDirectory", () => {
		...
		it("should skip Python virtual environments", () => {
			createTempFile(tmpDir, "src/file.py", "# TODO: task 1");
			createTempFile(
				tmpDir,
				".venv/lib/python3.12/site-packages/pkg/file.py",
				"# TODO: should be skipped",
			);
			...
			expect(result.items.length).toBe(1);
			expect(result.items[0].file).not.toContain(".venv");
		});


		it("should skip nested git repositories when scanning a parent workspace", () => {
			createTempFile(tmpDir, "workspace.ts", "// TODO: keep me");
			createTempFile(tmpDir, "nested-repo/.git/HEAD", "ref: refs/heads/main\n");
			createTempFile(
				tmpDir,
				"nested-repo/src/file.ts",
				"// TODO: should be skipped with nested repo",
			);

			const result = client.scanDirectory(tmpDir);

			expect(result.items.length).toBe(1);
			expect(result.items[0].file).toContain("workspace.ts");
			expect(result.items[0].file).not.toContain("nested-repo/src/file.ts");
		});
