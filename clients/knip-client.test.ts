...
describe("KnipClient", () => {
	...
	describe("isAvailable", () => {
		...
		it("should skip cleanly when the target directory has no package.json", () => {
			(client as any).isAvailable = () => true;

			const result = client.analyze(tmpDir);
			expect(result.success).toBe(true);
			expect(result.summary).toBe("Knip skipped: no package.json");
